...
def genPoint(distribution_type):
...
def rrt_search(G, tx, ty):
    ...
    while(1):
        num_iterations +=1
        pt_random = genPoint("uni") # call genPoint() to get samples from different distributions.
        ...
        pt_new = [bigger_line_to_pt_new[0]+pt_nearest[0], bigger_line_to_pt_new[1]+pt_nearest[1]] #get coord of new point
        ...
        if (pointPointDistance(pt_new, [tx,ty]) <= SMALLSTEP):
            child_node = index_pt_new
            while(1):
                parent_node = returnParent(child_node)
                canvas.polyline([vertices[child_node], vertices[parent_node]])
                rrt_length += pointPointDistance(vertices[child_node],vertices[parent_node])
                if (parent_node == 0):
                    break
